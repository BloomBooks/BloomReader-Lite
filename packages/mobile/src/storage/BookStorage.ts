...
export const OPEN_BOOK_DIR = FileSystem.cacheDirectory + "currentbook"; // BR-Android uses "currentbook", BR-RN used "openBook";
...
async function storageDirs() {
    ...
    if (Platform.OS === "android") {
        console.warn(
            "Skipping androidExternalStorageDirs, not implemented yet."
        );
...
export async function safeOpenBookForReading(bookFilePath: string) {
    if (bookFilePath.includes("%")) {
        const from = bookFilePath;
        ...
        bookFilePath = Path.join(FileSystem.cacheDirectory!, "temp.bloompub");
    ...
    return openBookForReading(bookFilePath);
}
